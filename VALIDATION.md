...
Google Sign-In configuration validation now verifies that every app configuration is attached to the XcodeGen target through `configFiles`, that the iOS/server client IDs and reversed callback scheme agree, and that `.xcconfig` URLs use an escaped slash form that Xcode does not parse as a comment. The Codemagic production workflow additionally fails before archive creation unless the resolved Production build settings contain the expected bundle ID, Google IDs, callback scheme, and HTTPS API URL. This closes the packaging defect that left the Google values unresolved in the built `Info.plist` and caused the app to report that Google Sign-In was unavailable.
