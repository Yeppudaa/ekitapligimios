...
The CI workflow now supports manual `workflow_dispatch` and push triggers for the repository's current `master` branch in addition to `main`, `codex/**`, and pull requests. This workspace has no git remote and all project contents remain uncommitted, so no macOS runner can be dispatched from the current state; that external repository handoff is still required for Xcode evidence.
