...
struct BookDetailView: View {
    ...
    var body: some View {
        ...
        .background {
            BookFileDeviceExportPresenter(fileURL: $exportFileURL)
        }
    }
    ...
    private func presentDeviceExport(for book: BookDTO) {
        guard exportFileURL == nil else { return }
        guard let local = container.downloadManager.localFile(for: book.id),
              FileManager.default.fileExists(atPath: local.url.path),
              let copy = try? BookFileDeviceExport.makeExportCopy(
                from: local.url,
                title: book.title,
                bookID: book.id,
                fileExtension: local.fileType
              ) else {
            return
        }
        exportFileURL = copy
