...
- MobileApi `1.0.84` deletion tests require current password for password-only users without persisting rejected attempts, while Apple-authorized users can submit without a password. Disposable user `56` proved a revoke outage returns a durable request with `apple_revocation_pending=true`, immediately revokes mobile sessions, and completes only after revocation succeeds; final cleanup left no user/session/test-account or retained PII evidence.
- Native post-deletion lifecycle validation requires `DeleteAccountView` to call the centralized container method. On accepted submission the app clears the password field, disables resubmission, stops StoreKit transaction observation, clears Keychain, dismisses any presented route, selects Account, and transitions to `signedOut`; static workspace validation enforces these controls.
